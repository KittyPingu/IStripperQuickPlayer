...
def make_fixture(ffmpeg, folder, frames=300, fps=30):
    ...
    if not source.is_file():
        run([str(ffmpeg), "-y", "-v", "error", "-f", "lavfi", "-i",
            ...
            str(source)], "Generating the deterministic 1080p benchmark fixture...")
...
def matanyone_cutoff(args, source, mask, work):
    ...
    run([sys.executable, str(benchmark), "--runtime", str(args.runtime),
        ...
        "Benchmarking MatAnyone2 eager and partially compiled execution...")
...
def sam_cutoffs(args, source, mask, work, defaults):
    ...
    run([sys.executable, str(benchmark), "--python", sys.executable,
        ...
        "Benchmarking installed SAM2 models and compilation modes...")
    ...
    for model in models:
        ...
        if not candidates:
            say(f"SAM2 {model} compilation did not pass its gate; retaining the current cutoff.")
            continue
