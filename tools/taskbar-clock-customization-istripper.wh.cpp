// ==WindhawkMod==
// @id              taskbar-clock-customization-istripper
// @name            Taskbar Clock Customization - iStripper
// @description     Taskbar Clock Customization with QuickPlayer now-playing support
// @version         1.8.0.1
// @author          m417z, KittyPingu
...
- IStripperQuickPlayer:
  - Port: 17871
    $name: REST API port
    $description: >-
      The local iStripper QuickPlayer REST API port used by %istripper%.
  - UpdateInterval: 1
    $name: Update interval
    $description: How often to refresh %istripper%, in seconds.
  $name: iStripper QuickPlayer
...
struct IStripperQuickPlayerSettings {
    int port;
    int updateInterval;
};
...
struct {
    ...
    IStripperQuickPlayerSettings iStripperQuickPlayer;
...
std::atomic<HANDLE> g_iStripperUpdateThread;
HANDLE g_iStripperUpdateStopEvent;
std::mutex g_iStripperMutex;
std::wstring g_iStripperFormatted;
...
std::wstring ReadQuickPlayerApiToken() {
    ...
    path += L"\\IStripperQuickPlayer\\api-token.txt";
    HANDLE file = CreateFile(path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr,
    ...
    char bytes[1024];
    DWORD bytesRead = 0;
    BOOL readSucceeded = ReadFile(file, bytes, sizeof(bytes), &bytesRead, nullptr);
    CloseHandle(file);
    if (!readSucceeded || !bytesRead) {
        return {};
    }

    int charsNeeded = MultiByteToWideChar(CP_UTF8, 0, bytes, bytesRead, nullptr, 0);
    std::wstring token(charsNeeded, L'\0');
    MultiByteToWideChar(CP_UTF8, 0, bytes, bytesRead, token.data(), charsNeeded);
    return std::wstring(TrimStringView(token));
}

std::optional<std::wstring> GetQuickPlayerStatus() {
    std::wstring token = ReadQuickPlayerApiToken();
    if (token.empty()) {
        return std::nullopt;
    }

    HINTERNET internet = InternetOpen(L"WindhawkMod", INTERNET_OPEN_TYPE_DIRECT,
                                      nullptr, nullptr, 0);
    if (!internet) {
        return std::nullopt;
    }
    ...
    DWORD timeout = 500;
    InternetSetOption(internet, INTERNET_OPTION_CONNECT_TIMEOUT, &timeout,
                      sizeof(timeout));
    InternetSetOption(internet, INTERNET_OPTION_SEND_TIMEOUT, &timeout,
                      sizeof(timeout));
    InternetSetOption(internet, INTERNET_OPTION_RECEIVE_TIMEOUT, &timeout,
                      sizeof(timeout));

    WCHAR url[128];
    swprintf_s(url, L"http://127.0.0.1:%d/api/v1/status",
               g_settings.iStripperQuickPlayer.port);
    std::wstring headers = L"Authorization: Bearer " + token + L"\r\n";
    HINTERNET request = InternetOpenUrl(
        internet, url, headers.c_str(), static_cast<DWORD>(headers.size()),
    ...
    if (!HttpQueryInfo(request, HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER,
                       &status, &statusSize, nullptr) || status != 200) {
    ...
    int charsNeeded = MultiByteToWideChar(CP_UTF8, 0, response.data(),
                                          static_cast<int>(response.size()),
                                          nullptr, 0);
    std::wstring result(charsNeeded, L'\0');
    MultiByteToWideChar(CP_UTF8, 0, response.data(),
                        static_cast<int>(response.size()), result.data(),
                        charsNeeded);
    return result;
}
...
void UpdateIStripperQuickPlayer() {
    std::wstring formatted;
    try {
        auto response = GetQuickPlayerStatus();
        if (response) {
            auto root = winrt::Windows::Data::Json::JsonObject::Parse(*response);
            auto playing = root.GetNamedObject(L"playing", nullptr);
            if (playing) {
                std::wstring state =
                    playing.GetNamedString(L"state", L"").c_str();
                if (state == L"playing" || state == L"paused") {
                    std::wstring model =
                        playing.GetNamedString(L"model", L"").c_str();
                    std::wstring show =
                        playing.GetNamedString(L"outfit", L"").c_str();
                    if (show.empty()) {
                        show = playing.GetNamedString(L"cardName", L"").c_str();
                    }

                    constexpr size_t kMaxShowTitleLength = 20;
                    if (show.length() > kMaxShowTitleLength) {
                        show.resize(kMaxShowTitleLength - 3);
                        show += L"...";
                    }

                    if (!model.empty() && !show.empty()) {
                        formatted = model + L": " + show;
                    } else if (!model.empty()) {
                        formatted = std::move(model);
                    } else {
                        formatted = std::move(show);
                    }
                }
            }
        }
    } catch (const winrt::hresult_error& ex) {
        Wh_Log(L"QuickPlayer status parse failed: %08X", ex.code().value);
    }
...
DWORD WINAPI IStripperUpdateThread(LPVOID) {
    while (true) {
        UpdateIStripperQuickPlayer();
        DWORD interval =
            std::max(g_settings.iStripperQuickPlayer.updateInterval, 1) * 1000;
        if (WaitForSingleObject(g_iStripperUpdateStopEvent, interval) ==
            WAIT_OBJECT_0) {
            break;
        }
    }
...
void IStripperUpdateThreadInit() {
    if (!IsStrInDateTimePatternSettings(L"%istripper%")) {
        return;
    }

    g_iStripperUpdateStopEvent = CreateEvent(nullptr, TRUE, FALSE, nullptr);
    g_iStripperUpdateThread = CreateThread(
        nullptr, 0, IStripperUpdateThread, nullptr, 0, nullptr);
}

void IStripperUpdateThreadUninit() {
    HANDLE thread = g_iStripperUpdateThread.exchange(nullptr);
    HANDLE stopEvent = g_iStripperUpdateStopEvent;
    g_iStripperUpdateStopEvent = nullptr;
    if (thread) {
        SetEvent(stopEvent);
        WaitForSingleObject(thread, INFINITE);
        CloseHandle(thread);
...
size_t ResolveFormatToken(
    ...
    struct {
        ...
        {L"%media%"sv, GetMediaInfoFormatted},
        {L"%istripper%"sv, GetIStripperFormatted},
...
void LoadSettings() {
    ...
    g_settings.iStripperQuickPlayer.port =
        Wh_GetIntSetting(L"IStripperQuickPlayer.Port");
    g_settings.iStripperQuickPlayer.updateInterval =
        Wh_GetIntSetting(L"IStripperQuickPlayer.UpdateInterval");
