...
def self_test():
    request = parse_request({"source": "x.mp4", "mask": "x.png",
                             "frameMs": 1234, "alphaThreshold": .65})
    assert request[2] == 1234 and abs(request[3] - .65) < .0001
...
def parse_request(value):
    ...
    return source, destination, frame_ms, threshold


def infer(value, torch, model, device, fp16, cache):
    import numpy as np
    source, destination, frame_ms, threshold = parse_request(value)
...
def main():
    ...
    for line in sys.stdin:
        try:
            ...
            destination, threshold, device_name, width, height, cached = infer(
